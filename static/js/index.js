...
exports.postAceInit = function (hook, context) {
  const $outerIframeContents = $('iframe[name="ace_outer"]').contents();
  const $innerIframe = $outerIframeContents.find('iframe');
  const $innerdocbody = $innerIframe.contents().find('#innerdocbody');
  const pdfURL = $('#exportpdfa').attr('href');
  ...
  $('#previewpdf').on('click', (e) => {
    e.preventDefault();
    previewPdf(pdfURL);
  });
  $('#previewpdfformal').on('click', (e) => {
    e.preventDefault();
