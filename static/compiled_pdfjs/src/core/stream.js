...
const PredictorStream = (function PredictorStreamClosure() {
  ...
  PredictorStream.prototype.readBlockPng =
      function predictorStreamReadBlockPng() {
        ...
        let i; let j = bufferLength; let up; let c;
        switch (predictor) {
          case 0:
            for (i = 0; i < rowBytes; ++i) {
              buffer[j++] = rawBytes[i];
...
const JpegStream = (function JpegStreamClosure() {
  ...
  JpegStream.prototype.ensureBuffer = function JpegStream_ensureBuffer(req) {
    ...
    try {
      ...
      if (this.forceRGB && this.dict && isArray(this.dict.get('Decode'))) {
        const decodeArr = this.dict.get('Decode');
      ...
      const data = jpegImage.getData(this.drawWidth, this.drawHeight,
          this.forceRGB);
      this.buffer = data;
  ...
  JpegStream.prototype.isNativelySupported =
      function JpegStream_isNativelySupported(xref, res) {
        const cs = ColorSpace.parse(this.dict.get('ColorSpace', 'CS'), xref, res);
...
const JpxStream = (function JpxStreamClosure() {
  ...
  JpxStream.prototype.ensureBuffer = function JpxStream_ensureBuffer(req) {
    ...
    if (tileCount === 1) {
      ...
      const data = new Uint8Array(width * height * componentsCount);
...
const Jbig2Stream = (function Jbig2StreamClosure() {
  ...
  Jbig2Stream.prototype.ensureBuffer = function Jbig2Stream_ensureBuffer(req) {
    ...
    const chunks = []; const
      xref = this.dict.xref;
    let decodeParams = xref.fetchIfRef(this.dict.get('DecodeParms'));
    ...
    if (decodeParams && decodeParams.has('JBIG2Globals')) {
      const globalsStream = decodeParams.get('JBIG2Globals');
...
const Ascii85Stream = (function Ascii85StreamClosure() {
  ...
  Ascii85Stream.prototype.readBlock = function Ascii85Stream_readBlock() {
    ...
    const bufferLength = this.bufferLength; let
      buffer;
    let i;
...
const CCITTFaxStream = (function CCITTFaxStreamClosure() {
  ...
  function CCITTFaxStream(str, maybeLength, params) {
    ...
    let eoblock = params.get('EndOfBlock');
...
const LZWStream = (function LZWStreamClosure() {
  ...
  LZWStream.prototype.readBlock = function LZWStream_readBlock() {
    const blockSize = 512;
    let estimatedDecodedSize = blockSize * 2; const
