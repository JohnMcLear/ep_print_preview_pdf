...
const ColorSpace = (function ColorSpaceClosure() {
  ...
  ColorSpace.prototype = {
    ...
    fillRgb: function ColorSpace_fillRgb(dest, originalWidth,
      ...
      if (this.isPassthrough(bpc)) {
        ...
        const allColors = bpc <= 8 ? new Uint8Array(numComponentColors)
          : new Uint16Array(numComponentColors);
        let key;
        for (i = 0; i < numComponentColors; i++) {
          allColors[i] = i;
        }
        const colorMap = new Uint8Array(numComponentColors * 3);
  ...
  ColorSpace.parse = function ColorSpace_parse(cs, xref, res) {
    const IR = ColorSpace.parseToIR(cs, xref, res);
  ...
  ColorSpace.parseToIR = function ColorSpace_parseToIR(cs, xref, res) {
    if (isName(cs)) {
      ...
      if (isDict(colorSpaces)) {
        const refcs = colorSpaces.get(cs.name);
        if (refcs) {
          cs = refcs;
        }
      }
    ...
    if (isName(cs)) {
      ...
      switch (mode) {
        ...
        case 'ICCBased':
          ...
          if (alt) {
            const altIR = ColorSpace.parseToIR(alt, xref, res);
            // Parse the /Alternate CS to ensure that the number of components
            // are correct, and also (indirectly) that it is not a PatternCS.
            const altCS = ColorSpace.fromIR(altIR);
...
var AlternateCS = (function AlternateCSClosure() {
  ...
  AlternateCS.prototype = {
    ...
    getRgbBuffer: function AlternateCS_getRgbBuffer(src, srcOffset, count,
      ...
      const baseBuf = isPassthrough ? dest : new Uint8Array(baseNumComps * count);
...
var DeviceCmykCS = (function DeviceCmykCSClosure() {
  ...
  function convertToRgb(src, srcOffset, srcScale, dest, destOffset) {
    const c = src[srcOffset + 0] * srcScale;
...
var CalGrayCS = (function CalGrayCSClosure() {
  ...
  function convertToRgb(cs, src, srcOffset, dest, destOffset, scale) {
    ...
    const val = Math.max(295.8 * L ** 0.333333333333333333 - 40.8, 0) | 0;
    dest[destOffset] = val;
...
var CalRGBCS = (function CalRGBCSClosure() {
  ...
  function convertToRgb(cs, src, srcOffset, dest, destOffset, scale) {
    ...
    const sR = sRGBTransferFunction(SRGB[0]);
    const sG = sRGBTransferFunction(SRGB[1]);
    const sB = sRGBTransferFunction(SRGB[2]);
