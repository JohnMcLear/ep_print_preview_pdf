...
const ARCFourCipher = (function ARCFourCipherClosure() {
  ...
  ARCFourCipher.prototype = {
    encryptBlock: function ARCFourCipher_encryptBlock(data) {
      let i; const n = data.length; let tmp; let tmp2;
      let a = this.a; let b = this.b; const
        s = this.s;
...
const calculateMD5 = (function calculateMD5Closure() {
  ...
  function hash(data, offset, length) {
    let h0 = 1732584193; let h1 = -271733879; let h2 = -1732584194; let
      h3 = 271733878;
    // pre-processing
    ...
    for (i = 0; i < paddedLength;) {
      ...
      let a = h0; let b = h1; let c = h2; let d = h3; var f; var g;
      for (j = 0; j < 64; ++j) {
        if (j < 16) {
          f = (b & c) | ((~b) & d);
          g = j;
        ...
        const tmp = d; const rotateArg = (a + f + k[j] + w[g]) | 0; const
          rotate = r[j];
        d = c;
        c = b;
...
const calculateSHA256 = (function calculateSHA256Closure() {
  ...
  function hash(data, offset, length) {
    ...
    for (i = 0; i < paddedLength;) {
      ...
      let f = h5; let g = h6; let h = h7; var t1; var t2;
      for (j = 0; j < 64; ++j) {
...
const calculateSHA512 = (function calculateSHA512Closure() {
  ...
  function hash(data, offset, length, mode384) {
    ...
    let a = new Word64(0, 0); let b = new Word64(0, 0); let
      c = new Word64(0, 0);
    let d = new Word64(0, 0); let e = new Word64(0, 0); let
      f = new Word64(0, 0);
    let g = new Word64(0, 0); let
      h = new Word64(0, 0);
    const t1 = new Word64(0, 0); const
      t2 = new Word64(0, 0);
    const tmp1 = new Word64(0, 0); const tmp2 = new Word64(0, 0); let
      tmp3;
...
const AES128Cipher = (function AES128CipherClosure() {
  ...
  const mixCol = new Uint8Array(256);
  for (var i = 0; i < 256; i++) {
    if (i < 128) {
      mixCol[i] = i << 1;
    } else {
  ...
  function expandKey128(cipherKey) {
    ...
    for (let j = 16, i = 1; j < b; ++i) {
      // RotWord
      let t1 = result[j - 3]; let t2 = result[j - 2];
      let t3 = result[j - 1]; let
        t4 = result[j - 4];
      // SubWord
      t1 = s[t1];
      t2 = s[t2];
  ...
  function decrypt128(input, key) {
    ...
    for (i = 9; i >= 1; --i) {
      ...
      // InvMixColumns
      for (j = 0; j < 16; j += 4) {
        const s0 = mix[state[j]]; const s1 = mix[state[j + 1]];
        const s2 = mix[state[j + 2]]; const
          s3 = mix[state[j + 3]];
  ...
  function encrypt128(input, key) {
    ...
    for (i = 1; i < 10; i++) {
      ...
      for (var j = 0; j < 16; j += 4) {
        const s0 = state[j + 0]; const
          s1 = state[j + 1];
        const s2 = state[j + 2]; const
          s3 = state[j + 3];
        t = s0 ^ s1 ^ s2 ^ s3;
  ...
  function decryptBlock2(data, finalize) {
    let i; let j; let ii; const sourceLength = data.length;
    let buffer = this.buffer; let bufferLength = this.bufferPosition;
    const result = []; let
      iv = this.iv;
    for (i = 0; i < sourceLength; ++i) {
      ...
      const plain = decrypt128(buffer, this.key);
  ...
  AES128Cipher.prototype = {
    ...
    encrypt: function AES128Cipher_encrypt(data, iv) {
      ...
      for (i = 0; i < sourceLength; ++i) {
        ...
        const cipher = encrypt128(buffer, this.key);
        iv = cipher;
        result.push(cipher);
        buffer = new Uint8Array(16);
        bufferLength = 0;
...
const AES256Cipher = (function AES256CipherClosure() {
  const rcon = new Uint8Array([
    0x8d,
    0x01,
    0x02,
  ...
  function expandKey256(cipherKey) {
    ...
    for (let j = 32, i = 1; j < b; ++i) {
      if (j % 32 === 16) {
        ...
        // RotWord
        var t1 = result[j - 3]; var t2 = result[j - 2];
        var t3 = result[j - 1]; var
          t4 = result[j - 4];
        // SubWord
        t1 = s[t1];
        t2 = s[t2];
  ...
  function decryptBlock2(data, finalize) {
    ...
    for (i = 0; i < sourceLength; ++i) {
      ...
      // buffer is full, decrypting
      const plain = decrypt256(buffer, this.key);
      // xor-ing the IV vector to get plain text
      for (j = 0; j < 16; ++j) {
        plain[j] ^= iv[j];
      }
  ...
  AES256Cipher.prototype = {
    ...
    encrypt: function AES256Cipher_encrypt(data, iv) {
      ...
      for (i = 0; i < sourceLength; ++i) {
        ...
        // buffer is full, encrypting
        const cipher = encrypt256(buffer, this.key);
        this.iv = cipher;
        result.push(cipher);
        buffer = new Uint8Array(16);
        bufferLength = 0;
...
const CipherTransformFactory = (function CipherTransformFactoryClosure() {
  ...
  function prepareKeyData(fileId, password, ownerPassword, userPassword,
    ...
    const hashData = new Uint8Array(hashDataSize); let i = 0; let j; let n;
    if (password) {
    ...
    if (revision >= 3) {
      ...
      const derivedKey = new Uint8Array(n); let
        k;
      for (j = 1; j <= 19; ++j) {
        for (k = 0; k < n; ++k) {
  ...
  function decodeUserPassword(password, ownerPassword, revision, keyLength) {
    const hashData = new Uint8Array(32); let i = 0; let j; let n;
    n = Math.min(32, password.length);
    for (; i < n; ++i) {
      hashData[i] = password[i];
    }
    ...
    if (revision >= 3) {
      ...
      const derivedKey = new Uint8Array(keyLengthInBytes); let
        k;
      for (j = 19; j >= 0; j--) {
  ...
  function CipherTransformFactory(dict, fileId, password) {
    ...
    const keyLength = dict.get('Length') || 40;
    if (!isInt(keyLength) ||
    ...
    if (!encryptionKey && !password) {
      ...
      const decodedPassword = decodeUserPassword(passwordBytes, ownerPassword,
          revision, keyLength);
  ...
  function buildObjectKey(num, gen, encryptionKey, isAes) {
    const key = new Uint8Array(encryptionKey.length + 9); let i; let n;
