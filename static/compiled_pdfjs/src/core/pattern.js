...
Shadings.RadialAxial = (function RadialAxialClosure() {
  function RadialAxial(dict, matrix, xref, res) {
    ...
    let t0 = 0.0; let
      t1 = 1.0;
    if (dict.has('Domain')) {
      const domainArr = dict.get('Domain');
      t0 = domainArr[0];
      t1 = domainArr[1];
    }

    let extendStart = false; let
      extendEnd = false;
    if (dict.has('Extend')) {
      const extendArr = dict.get('Extend');
    ...
    const fn = PDFFunction.parseArray(xref, fnObj);
    ...
    const color = new Float32Array(cs.numComps); const
      ratio = new Float32Array(1);
    let rgbColor;
    for (let i = t0; i <= t1; i += step) {
      ratio[0] = i;
      fn(ratio, 0, color, 0);
...
Shadings.Mesh = (function MeshClosure() {
  ...
  MeshStreamReader.prototype = {
    ...
    readComponents: function MeshStreamReader_readComponents() {
      ...
      const scale = bitsPerComponent < 32 ? 1 / ((1 << bitsPerComponent) - 1)
        : 2.3283064365386963e-10; // 2 ^ -32
      const decode = this.context.decode;
  ...
  const getB = (function getBClosure() {
    function buildB(count) {
      ...
      for (let i = 0; i <= count; i++) {
        const t = i / count; const
          t_ = 1 - t;
        lut.push(new Float32Array([t_ * t_ * t_,
          3 * t * t_ * t_,
          3 * t * t * t_,
          t * t * t]));
      }
  ...
  function buildFigureFromPatch(mesh, index) {
    ...
    const coords = mesh.coords; const
      colors = mesh.colors;
    const pi = figure.coords;
    const ci = figure.colors;

    const figureMinX = Math.min(coords[pi[0]][0], coords[pi[3]][0],
        coords[pi[12]][0], coords[pi[15]][0]);
    const figureMinY = Math.min(coords[pi[0]][1], coords[pi[3]][1],
        coords[pi[12]][1], coords[pi[15]][1]);
    const figureMaxX = Math.max(coords[pi[0]][0], coords[pi[3]][0],
        coords[pi[12]][0], coords[pi[15]][0]);
    const figureMaxY = Math.max(coords[pi[0]][1], coords[pi[3]][1],
        coords[pi[12]][1], coords[pi[15]][1]);
    let splitXBy = Math.ceil((figureMaxX - figureMinX) * TRIANGLE_DENSITY /
                             (mesh.bounds[2] - mesh.bounds[0]));
    splitXBy = Math.max(MIN_SPLIT_PATCH_CHUNKS_AMOUNT,
        Math.min(MAX_SPLIT_PATCH_CHUNKS_AMOUNT, splitXBy));
    let splitYBy = Math.ceil((figureMaxY - figureMinY) * TRIANGLE_DENSITY /
    ...
    let k = 0;
    const cl = new Uint8Array(3); const
      cr = new Uint8Array(3);
    const c0 = colors[ci[0]]; const c1 = colors[ci[1]];
    const c2 = colors[ci[2]]; const
      c3 = colors[ci[3]];
    const bRow = getB(splitYBy); const
      bCol = getB(splitXBy);
  ...
  function updateBounds(mesh) {
    let minX = mesh.coords[0][0]; let minY = mesh.coords[0][1];
    let maxX = minX; let
      maxY = minY;
    for (let i = 1, ii = mesh.coords.length; i < ii; i++) {
      const x = mesh.coords[i][0]; const
        y = mesh.coords[i][1];
      minX = minX > x ? x : minX;
      minY = minY > y ? y : minY;
      maxX = maxX < x ? x : maxX;
  ...
  function Mesh(stream, matrix, xref, res) {
    ...
    const fnObj = dict.get('Function');
    const fn = fnObj ? PDFFunction.parseArray(xref, fnObj) : null;
