...
const rasterizeTextLayer = (function rasterizeTextLayerClosure() {
  ...
  function rasterizeTextLayer(ctx, viewport, textContent) {
    return new Promise((resolve) => {
      ...
      const stylePromise = getTextLayerStyle();
      ...
      Promise.all([stylePromise, task.promise]).then((results) => {
        style.textContent = results[0];
        svg.appendChild(foreignObject);

        // We need to have UTF-8 encoded XML.
        const svg_xml = unescape(encodeURIComponent(
 ...
 * @class
 */
const Driver = (function DriverClosure() {
  /**
   * @constructs Driver
  ...
  Driver.prototype = {
      ...
      r.onreadystatechange = function () {
        if (r.readyState === 4) {
          ...
          if (self.testFilter && self.testFilter.length) {
            self.manifest = self.manifest.filter((item) => self.testFilter.indexOf(item.id) !== -1);
          }
          self.currentTask = 0;
          self._nextTask();
        }
    ...
    _nextTask: function Driver_nextTask() {
      ...
      const absoluteUrl = combineUrl(window.location.href, task.file);
      ...
      if (!failure) {
        try {
          ...
          task.pdfDoc.getPage(task.pageNum).then((page) => {
            const viewport = page.getViewport(PDF_TO_CSS_UNITS);
            ...
            if (task.type === 'text') {
              ...
              initPromise = page.getTextContent().then((textContent) => rasterizeTextLayer(textLayerContext, viewport,
                  textContent));
            } else {
            ...
            const completeRender = (function (error) {
